...
var_removed=$( grep "variants removed" ${output}.log | awk '{ print $1 }' )
if [ $var_removed -gt 0 ]; then printf "Warning: There were variants removed for missingness from the imputed set when subsetting to ${subset_label}. This is unusual! Please check the imputed files!\n" ; fi
