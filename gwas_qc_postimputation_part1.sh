...
Usage:
SCRIPTNAME.sh -o [output_stem] -i [imputation_results_folder] -r [race_file] -f [sex_file] -s [snp_names_file] -g [preimputation_geno] -w [snpweights_file] -z -x -c
...
skip_cleanup='false'
while getopts 'o:i:r:f:s:g:w:zxch' flag; do
  case "${flag}" in
...
if [ -z "$output_stem" ] || [ -z "$imputation_results_folder" ] || [ -z "$race_file" ] || [ -z "$sex_file" ] || [ -z "$snp_names_file" ] || [ -z "$preimputation_geno" ] || [ -z "$snpweights_file" ];
then
