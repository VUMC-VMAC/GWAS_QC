...
print(paste("There are", sum(anc$Ancestry == "Admixed"), "admixed individuals,", sum(anc$Ancestry == "European"), "European,", sum(anc$Ancestry == "AA"), "African American, and", sum(anc$Ancestry == "Latino_Hispanic"), "Latino/Hispanic ancestry."))
...
print(paste0("Lists of IDs in AA, EUR, and Latino/Hispanic subsets have been written to ", filestem, "_*_keep.txt. Use these to subset the cohort before finishing post-imputation QC."))